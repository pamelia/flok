//! `MiniMax` M2.7 provider (Anthropic-compatible API).
//!
//! Implements streaming via the Anthropic-compatible Messages API with SSE.
//! Supports tool use and prompt caching via `cache_control` — identical to Anthropic's
//! explicit caching mechanism.
...
impl MiniMaxProvider {
    ...
    /// Build the Anthropic-compatible request body from a `CompletionRequest`.
...
/// Parse an Anthropic-compatible SSE event into a `StreamEvent`.
...
// ---------------------------------------------------------------------------
// Anthropic-compatible API types (private, for serialization only)
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn parse_message_start_with_usage() {
        let data = r#"{"type":"message_start","message":{"id":"msg_1","type":"message","role":"assistant","content":[],"model":"MiniMax-M2.7","stop_reason":null,"stop_sequence":null,"usage":{"input_tokens":1234,"output_tokens":0,"cache_read_input_tokens":500,"cache_creation_input_tokens":100}}}"#;
